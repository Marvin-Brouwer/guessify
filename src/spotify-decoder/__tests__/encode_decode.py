...
##
# This file has been copied from github.
# See https://github.com/boonepeter/boonepeter.github.io-code/blob/main/spotify-codes-part-2/src/encode_decode.py
# for the original
##
...
def check_spotify_crc2(bin45):
    data = bin_to_bytes(bin45,37)
    crcValue = bin_to_bytes(bin45[37:], 8)[0]
    crcInt = spotify_crc.calc(data)

    return json.dumps({
        'data': (np.array(data).astype(int).tolist()),
        'crcValue': (np.array(crcValue).astype(int).tolist()),
        'crcInt': (crcInt),
	}, default=str)
...
# Given 20 (clean) barcode levels, returns media reference
def spotify_bar_decode_test(levels):
    level_bits = np.array([gray_code_inv[levels[i]] for i in range(20)], dtype=bool).flatten()
    conv_bits = [level_bits[43*i % 60] for i in range(60)]
    cols = [i for i in range(60) if i % 4 != 2] # columns to invert
    conv_bits45 = np.array([conv_bits[c] for c in cols], dtype=bool)
    ...
    return json.dumps({
        'levelBits': (level_bits.astype(int).tolist()),
        'convBits': (np.array(conv_bits).astype(int).tolist()),
        'cols': (cols),
        'convBits45': (conv_bits45.astype(int).tolist()),
        'bin45': (np.array(bin45).astype(int).tolist())
	}, default=str)